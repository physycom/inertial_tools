...
class Filter {
  ...
  Filter(const size_t& data_size, const std::string& filter_path)
  {
    ...
    if (!filter_file) {
      std::cerr << "ERROR: file " << filter_path << " could not be opened" << std::endl;
      exit(1);
    }
    while (!filter_file.eof()) {
      double temp;
      if (filter_file.fail()) {
        std::cerr << "ERROR: Invalid filter data, check filter file " << filter_path << " and relaunch." << std::endl;
        exit(2);
      }
      filter_file >> temp;
    ...
    for (auto p : coeff)
      normalization += p;
    for (auto p : coeff)
      params.push_back(p / normalization);
...
{
  std::cout << "Data Purge v" << MAJOR_VERSION << "." << MINOR_VERSION << std::endl
            << std::endl;
  ...
  if (input_file.substr(0, 2) == ".\\" || input_file.substr(0, 2) == "./")
  ...
  if (subtract_ave) {
    std::vector<double> averages(data[0].size(), 0.0);
  ...
  // Filtering
  std::vector<std::vector<double>> data_purged;
  for (size_t i = f.first; i <= f.last; i++) {
    std::vector<double> row_purged;
