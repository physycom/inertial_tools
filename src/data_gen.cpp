...
int main(int argc, char **argv) {
  cout << "Data Generator v" << MAJOR << "." << MINOR << endl;

  vector<double> t, a_x, a_y, v_x, v_y, r_x, r_y;
  vector<double> theta, Omega;
  complex<double> IU(0, 1);   // Imaginary Unit
  ...
  vector< complex<double> > a_loc, a_fix;
  vector<double> a_t, a_n;
  for (size_t i = 0; i < t.size(); i++) {
    a_fix.push_back(complex<double>(a_x[i], a_y[i]));
  ...
  for (size_t i = 0; i < t.size(); i++) fprintf(out_data, "%16.10lf %16.10lf %16.10lf %16.10lf %16.10lf %16.10lf %16.10lf %16.10lf\n",
