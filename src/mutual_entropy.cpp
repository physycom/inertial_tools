...
double entropy(const std::vector<double>& p)
{
  double e = 0.;
  for (auto& pi : p)
    if (pi > std::numeric_limits<double>::epsilon())
      e += pi * log(pi);
  return -e;
}

double mutual_entropy(const std::vector<std::vector<double>>& pxy)
{
  ...
  for (size_t i = 0; i < pxy.size(); i++) {
    for (size_t j = 0; j < pxy[i].size(); j++) {
      if (!(pxy[i][j] > std::numeric_limits<double>::epsilon()))
        continue;
...
void usage(char* progname)
{
  ...
  std::cerr << "Usage: " << tokens.back() << " [BIN_MODE] [SHIFT_MODE] path/to/file" << std::endl;
  std::cerr << "       [BIN_MODE] (mandatory)" << std::endl;
  std::cerr << "                 -bf <bin_fraction> : DYNAMIC binning, represents the fraction of data in each bin (e.g. 0.05 corresponds to 5%)." << std::endl;
  std::cerr << "                 -bn <bin_number>   : FIXED binning, represents the number of bins for each set of data." << std::endl;
  std::cerr << "       [SHIFT_MODE] (optional)" << std::endl;
  std::cerr << "                 -s <shift>             : SINGLE shift, represents the shift in indices between GZ and AX,AY (negative values allowed, safe)" << std::endl;
  std::cerr << "                 -ss <min> <max> <incr> : SCAN shift, safe and non UNIX sed compliant" << std::endl;
  std::cerr << "       path/to/data/file must be a PHYSYCOM INERTIAL STANDARD" << std::endl
            << std::endl;
}

int main(int argc, char** argv)
{
  std::cout << "Mutual Entropy Calculator v" << MAJOR_VERSION << "." << MINOR_VERSION << std::endl;
  std::string input_file;
  double bin_fraction;
  int bin_number;
  std::vector<int> index_shifts;
  char mode;
  if (argc > 3) {
    for (int i = 0; i < argc; i++) {
      if (std::string(argv[i]) == "-bf") {
        mode = DYNAMIC_BIN;
        try {
          bin_fraction = stod(std::string(argv[++i]));
      ...
      if (std::string(argv[i]) == "-bn") {
        mode = FIXED_BIN;
        try {
          bin_number = stoi(std::string(argv[++i]));
        } catch (std::exception& e) {
          std::cerr << "EXCEPTION: " << e.what() << std::endl;
          usage(argv[0]);
          exit(-5);
      ...
      if (std::string(argv[i]) == "-s") {
        try {
          index_shifts.push_back(stoi(std::string(argv[++i])));
        } catch (std::exception& e) {
          std::cerr << "EXCEPTION: " << e.what() << std::endl;
          usage(argv[0]);
          exit(-6);
      ...
      if (std::string(argv[i]) == "-ss") {
        try {
          int start_shift = stoi(std::string(argv[++i]));
          int last_shift = stoi(std::string(argv[++i]));
          int scan_shift = stoi(std::string(argv[++i]));
          ...
          if (scan_shift < 1) {
            std::cerr << "Invalid scan shift, reset to 1" << std::endl;
            scan_shift = 1;
          }
          for (int k = start_shift; k <= last_shift; k += scan_shift)
            index_shifts.push_back(k);
        } catch (std::exception& e) {
          std::cerr << "EXCEPTION: " << e.what() << std::endl;
          usage(argv[0]);
          exit(-7);
  ...
  if (!data.size()) {
    std::cerr << "ERROR: Empty file " << input_file << std::endl;
    exit(-4);
  }

  if (file_tokens.size() != data.size()) {
    std::cout << "WARNING: original record " << file_tokens.size() << ", used " << data.size() << " ( " << int(100.0 * (double)data.size() / file_tokens.size()) << " % )" << std::endl;
    std::cout << "WARNING: skipped " << file_tokens.size() - data.size() << " lines due to NaN presence." << std::endl;
  }

  std::vector<double> me_ax_gz(index_shifts.size(), std::numeric_limits<double>::signaling_NaN());
  std::vector<double> me_ay_gz(index_shifts.size(), std::numeric_limits<double>::signaling_NaN());
  std::vector<double> e_ax(index_shifts.size(), std::numeric_limits<double>::signaling_NaN());
  std::vector<double> e_ay(index_shifts.size(), std::numeric_limits<double>::signaling_NaN());
  std::vector<double> e_gz(index_shifts.size(), std::numeric_limits<double>::signaling_NaN());
  std::vector<int> bin_pop(index_shifts.size(), std::numeric_limits<int>::signaling_NaN());

  std::vector<double> ax_, ay_, gz_;
  for (auto line : data) {
...
#pragma omp parallel for
  for (int n = 0; n < index_shifts.size(); n++) {
    if (abs(index_shifts[n]) > (int)ax_.size() / 2) {
      //std::cout << "Index shift " << index_shifts[n] << " is too big, the upper bound is: " << ax_.size() / 2 << std::endl;
      continue;
    }
    ...
    // binning
    std::vector<std::vector<int>> counter_ax_gz, counter_ay_gz;
    int record_per_bin = 1;
    switch (mode) {
    case DYNAMIC_BIN: {
      // bin construction
      std::vector<double> ax_sorted(ax), ay_sorted(ay), gz_sorted(gz);
      std::sort(ax_sorted.begin(), ax_sorted.end());
      std::sort(ay_sorted.begin(), ay_sorted.end());
      std::sort(gz_sorted.begin(), gz_sorted.end());

      record_per_bin = int(ax.size() * bin_fraction);
      bin_number = int(1. / bin_fraction) + 1;

      std::vector<double> ax_pivot, ay_pivot, gz_pivot;
      ...
      counter_ax_gz.resize(bin_number);
      for (auto& i : counter_ax_gz)
        i.resize(bin_number, 0);
      counter_ay_gz.resize(bin_number);
      for (auto& i : counter_ay_gz)
        i.resize(bin_number, 0);

      size_t ax_index, ay_index, gz_index;
      for (size_t i = 0; i < ax.size(); i++) {
        for (size_t j = 0; j < ax_pivot.size(); j++)
          if (ax[i] <= ax_pivot[j]) {
            ax_index = j;
            break;
          }
        for (size_t j = 0; j < ay_pivot.size(); j++)
          if (ay[i] <= ay_pivot[j]) {
            ay_index = j;
            break;
          }
        for (size_t j = 0; j < gz_pivot.size(); j++)
          if (gz[i] <= gz_pivot[j]) {
            gz_index = j;
            break;
          }
    ...
    case FIXED_BIN: {
      ...
      counter_ax_gz.resize(bin_number);
      for (auto& i : counter_ax_gz)
        i.resize(bin_number, 0); // improve this by maybe splitting the switch
      counter_ay_gz.resize(bin_number);
      for (auto& i : counter_ay_gz)
        i.resize(bin_number, 0);
    ...
    std::vector<double> p_ax(bin_number, 0.0);
    std::vector<double> p_ay(bin_number, 0.0);
    std::vector<double> p_gz(bin_number, 0.0);
    std::vector<std::vector<double>> p_ax_gz(bin_number, std::vector<double>(bin_number, 0.));
    std::vector<std::vector<double>> p_ay_gz(bin_number, std::vector<double>(bin_number, 0.));
  ...
  results << "## Shift scan @ " << ((mode == DYNAMIC_BIN) ? "bin_fraction : " + std::to_string(bin_fraction) : "bin_number : " + std::to_string(bin_number)) << " # Data samples : " << ax_.size() << std::endl;
  results << "# shift # Entropy AX # Entropy AY # Entropy GZ # Mutual AX-GZ # Mutual AY-GZ # bin population #" << std::endl;
  ...
  std::ofstream gnuplot(gnuplot_filename);
  gnuplot << R"(#!/gnuplot
FILE_IN=')"
          << results_filename << R"('
FILE_OUT=')"
