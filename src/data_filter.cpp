...
{
  std::cout << "Data Filter v" << MAJOR_VERSION << "." << MINOR_VERSION << std::endl
            << std::endl;
  ...
  std::vector<std::vector<double>> data_filtered;
  for (size_t i = 0; i < data.size(); i++) {
    if (data[i][col_index] > min && data[i][col_index] < max)
