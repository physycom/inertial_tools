...
for shift in ${shift_values[@]}; do
  mutual_entropy.exe -bf $bin_fraction -s $shift data.txt > scan_shift.log
  cat scan_shift.log
    
  values=($(cat scan_shift.log | grep = ))
  
  echo -e "$shift\t${values[3]}\t${values[7]}\t${values[11]}\t${values[15]}\t${values[19]}\t${values[23]}" >> scan_shift.dat
done
